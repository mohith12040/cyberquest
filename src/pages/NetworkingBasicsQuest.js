// src/pages/NetworkingBasicsQuest.js
...
function NetworkingBasicsQuest({ session }) {
  const challenge = sampleChallenges.find((q) => q.id === 2);
  ...
  useEffect(() => {
    ...
    if (showResults && playerData && questions.length === answers.length) {
      updateXP();
    }
  }, [showResults]);
...
export default NetworkingBasicsQuest;
